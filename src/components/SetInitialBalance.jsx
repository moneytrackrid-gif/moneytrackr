...
export default function SetInitialBalance({ onClose }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    await updateWallet({ balance: raw })
    setSaving(false)
    onClose()
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        width: '100%', maxWidth: 400, border: '0.5px solid var(--border)',
      }}>
        {/* Icon */}
        <div style={{ width: 52, height: 52, borderRadius: 14, background: 'var(--mint-dim)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, marginBottom: 16 }}>
          🏦
        </div>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 24, lineHeight: 1.5 }}>
          Masukkan jumlah uang yang kamu punya sekarang. Ini akan jadi titik awal pencatatan keuangan kamu.
        </p>

        {/* Input */}
        <div style={{ marginBottom: 20 }}>
          <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>
            Saldo saat ini
          </label>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => handleSave()} disabled={saving} style={{
            flex: 1, padding: 14, background: 'var(--mint)', border: 'none',
